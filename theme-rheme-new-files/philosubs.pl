...
# Then we want in order, numbered from ZERO, the goodies we will format.
	...
	my $cleanpersabbrev;
    	...
    	my $persabbrev = &getpersabbrev($c . ".xml");
	$cleanpersabbrev = $persabbrev;
	my $persid = &getpersid($c . ".xml");
    	$persabbrev =~ s/ /%20/g; 
       	$tnav = "<a href=/$PERSEUSCGI/" . $MILENAVIGATE . "?dbname=" . $dbname . "&query=" . $persabbrev . "&getid=" . $persid . ">"; 
#       $tnav .= "." . $n . "\">";
       	$title = $tnav . $title . "</a>";
...
#        $ret .= ", [" . $createdate . "]";
#        }
    $ret .= "<a href=" . $PHILOGETOBJECT . "?c.$n:0.$dbname>XML Header</a>";
    $ret .= ")";
    if ($extent) {
	$extent =~ s/<fs\/>/; /g;
	$ret .= ", " . $extent;
    ...
    $PHILOGETLEMMACOUNT = $PHILOCGI . "/getlemmacount.pl";
    $getcount = "<a href=\"$PHILOGETLEMMACOUNT" . "?" . $dbname . "." . $n;
    $getcount .= "\">";
    $ret .= " [" . $getcount . $philomessage[681] . "</a>]";

    $ret .= " [<b>" . $cleanpersabbrev . "</b>]. ";
...
# General text and title page tags....
	...
	s/<\/titlepage>/<\/span>/gi;
	s/<titleref[^>]*>/<span class=titleref>/gi;
	s/<\/titleref>/<\/span><br>/gi;
...
#	s/<\/speaker>/ /gi;
        ...
        s/<\/speaker>/<\/span>/gi;
	s/<sp who=".*?" rend="none">//gi;
        s/<sp who="(.*?)">/<span class=speaker>$1<\/span>/gi;
	s/<roledesc[^>]*>/<span class=roledesc>/gi;
        s/<\/roledesc>/<\/span>/gi;
        s/<role[^>]*>/<span class=role>/gi;
        s/<\/role>/<\/span>/gi;
        s/<l n=\"([^\"]*)\">/<span class=mstonen>$1<\/span>/gi;
...
# Add a couple of end spans just in case.
	$text .= "</span>\n</span>";
return "<div id=perseuscontent>" . $text . "</div>";
}
...
sub objectfooter {
local ($txt);
$txt =  "</div><hr noshade>" . &getbiblioLine ($doc,"link") . "<hr noshade>\n";
$txt .= &readfooternavbar;
return "$txt";
}
...
#Run the milestone lookup.  Slow.
my $milestonehits = pershits();
    if ($milestonehits) {
    	$headline = "$mvonewcite ";
	    $headline .= "[ " . $milestonehits . "]<br>";
    }
...
# ATE version of all of this as well.
	...
	$div1 = "Section" unless $div1;
	$div2 = "SubSect" unless $div2;
	$div3 = "Sub2Sect" unless $div3;
	$headline = "$mvonewcite ";
...
# If you don't have a page tag, don't give a page link.
	if (!$pagenum || $pagenum eq "?" || $pagenum eq "na") {
	    ...
	    $headline .= "[page $pref $pagenum</a> |\n";
	}
        $headline .= $ohref[3] . "Paragraph</a> |\n";
...
# keys to see if it is the key for opening the external dico
	...
	close NAVBAR;
	my $uniword = uri_unescape($word);
        $navigbar =~ s/<title>Greek Texts & Translations/<title>Perseus under Philologic: searching for $uniword/;
	return $navigbar;
}
...
# Break Apostrophes if set on.
    ...
    $bf =~ s/&sect;/ /g;
    $bf =~ s/(["?,;:!\.\-])/ /g;
    $bf =~ s/(\xc2\xb7|\xce\x87|\xe2\x80\x94)/ /g;
    $bf =~ s/\xe2\x80[\x9c\x9d]/ /g;
...
# Look up the milestone off the byte offset.
	my $milestonehit = &kwicpershits();
...
# page if we have a page
	...
	if ($milestonehit) {
		$title = "<tt>$counter. $milestonehit</tt>";
	}
return $title;
}
...
#                return "<br>";
        ...
        if ($unit =~ /^line/i) {
                return "<span class=mstonen>$n</span>";
                }
        if ($unit =~ /para/i) {
                 return "\n<p>";
                }
...
#                return "";
        ...
        if ($unit =~ /^chapter/i) {
                return "<p><span class=mstonen2>ch. $n</span>";
#                 return "";
                }
        if ($unit =~ /^bekker page/i) {
                 return "<p><span class=mstonen>$n</span>";
}
        if ($unit =~ /^bekker line/i) {
                return "<span class=mstonen>$n</span>";
}
#	if ($n) { 
...
sub SetThemeLimit {
	...
	if (defined $FRONT_LIMIT && defined $LAST_LIMIT) {

		return ($FRONT_LIMIT, 100 - $LAST_LIMIT);
...
sub CleanKwicSortKey {
	...
	$line =~ s/\xe1[\xbc\xbe][\x80-\x8f]/\xce\xb1/gi; ## alphas
	$line =~ s/\xe1\xbd[\xb0-\xb1]/\xce\xb1/gi;
	$line =~ s/\xe1\xbe[\xb0-\xbc]/\xce\xb1/gi;
	$line =~ s/\xce[\xac\x86]/\xce\xb1/gi;
	$line =~ s/\xe1\xbc[\x90-\x9d]/\xce\xb5/gi; ## epsilons
	$line =~ s/\xe1\xbd[\xb2-\xb3]/\xce\xb5/gi; 
	$line =~ s/\xce[\xad\x88]/\xce\xb5/gi; 
	$line =~ s/\xe1\xbf[\x88-\x89]/\xce\xb5/gi;
	$line =~ s/\xe1\xbc[\xa0-\xaf]/\xce\xb7/gi; ## etas
	$line =~ s/\xe1\xbd[\xb4\xb5]/\xce\xb7/gi;
	$line =~ s/\xe1\xbe[\x90-\x9f]/\xce\xb7/gi;
	$line =~ s/\xe1\xbf[\x82-\x87\x8a-\x8c]/\xce\xb7/gi;
	$line =~ s/\xce[\xae\x89]/\xce\xb7/gi;
	$line =~ s/\xe1\xbc[\xb0-\xbf]/\xce\xb9/gi; ## iotas
	$line =~ s/\xe1\xbf[\x90-\x9b]/\xce\xb9/gi;
	$line =~ s/\xce[\xaf\x90\xaa\x8a]/\xce\xb9/gi;
	$line =~ s/\xe1\xce\x85/\xce\xb9/gi; #new one
	$line =~ s/\xcf\x8a/\xce\xb9/gi;
	$line =~ s/\xe1\xbd\xb6/\xce\xb9/gi;
	$line =~ s/\xe1\xbd[\x80-\x8d\xb8\xb9]/\xce\xbf/gi; ## omicrons
	$line =~ s/\xe1\xbf[\xb8-\xb9]/\xce\xbf/gi;
	$line =~ s/[\xce-\xcf]\x8c/\xce\xbf/gi;
	$line =~ s/\xe1\xbd[\xba\sbb\x90-\x9f]/\xcf\x85/gi; ## upsilons
	$line =~ s/\xe1\xbf[\xa0-\xa3\xa6-\xab]/\xcf\x85/gi;
	$line =~ s/\xcf[\x8b\x8d]/\xcf\x85/gi;
	$line =~ s/\xce[\xb0\xab\x8e]/\xcf\x85/gi;
	$line =~ s/\xe1[\xbd\xbe][\xa0-\xaf]/\xcf\x89/gi; ## omegas
	$line =~ s/\xe1\xbd[\xbc\xbd]/\xcf\x89/gi;
	$line =~ s/\xe1\xbf[\xb2-\xb7\xba-\xbc]/\xcf\x89/gi;
	$line =~ s/\xcf[\x8f\x8e]/\xcf\x89/gi; 
	$line =~ s/\xce\x8f/\xcf\x89/gi;
	$line =~ s/\xe1\xbf[\xa4\xa5]/\xcf\x81/gi; ## rhos
	$line =~ s/\xe1\xbf\xac/\xcf\x81/gi; 

    ## a few latin longs and shorts to ditch entirely for searching
	$line =~ s/\xc4\x81/a/gi;	## a macron
	$line =~ s/\xc4\x83/a/gi;	## a breve
	$line =~ s/\xc4\x93/e/gi;	## e macron
	$line =~ s/\xc4\x95/e/gi;	## e breve
	$line =~ s/\xc4\xab/i/gi;	## i macron
	$line =~ s/\xc4\xad/i/gi;	## i breve
	$line =~ s/\xc5\x8d/o/gi;	## o macron
	$line =~ s/\xc5\x8f/o/gi;	## o breve
	$line =~ s/\xc5\xab/u/gi;	## u macron
	$line =~ s/\xc5\xad/u/gi;	## u breve

    ## Take out punctuation (except ' for abbreviations)

	$line =~ s/\&[^;]*;//gi;
	$line =~ s/[\&;\.,"¬']/ /gi;
	$line =~ s/\xc2\xb4/ /gi; #´ was in the above punct. list, but matched \xce\xb4 for some reason
	$line =~ s/\xce\x87//gi;
	$line =~ s/\xc2\xb7//gi;
	$line =~ s/\xe2\x80[\x9c\x9d\x99]//gi;
	$line =~ s/\xc2[\xab\xbb]//gi;
	$line =~ s/[0-9]/ /g;
	
   #Removed any malformed utf8...
   	$line = decode("utf8", $line, 0);
    	$line =~ s/\x{FFFD}//g;
    	$line = encode("utf8", $line, 0);
